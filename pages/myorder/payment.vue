		...
		<view class="payment_content">
			<view class="payment_top flexs">
				<view class="payment_top_inner">
					<view>订单金额</view>
					<text class="payment_money flexs">¥<text class="number">{{list.actual_price}}</text></text>
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			tozhifu() {
				...
				// 	type: 4
				// };
				// this.$common.requests('/api/order/update', obj).then(res => {
				// 	this.$api.msg(res.data.msg);
				// 	if (res.data.code == 1) {
				// 		//this.show = false
				// 		// this.list = res.data.data
				// 		setTimeout(() => {
				// 			uni.navigateTo({
				// 				url: '../order/paySuccess?price=' + this.price
				// 			})
				// 		}, 1000)
				// 	}
				// });
				...
				this.$common.requests('/addons/epay/index/experience',obj,'GET').then(res=>{
					
					this.payment(res.data,'','');
					console.log(res.data,'可以获取到参数吗？')
					// console.log(res.data.appId,'可以获取到openid？')
					// this.$wexinPay(res.data,(res)=>{
					// 	console.log(res,666)
					// })
				})
