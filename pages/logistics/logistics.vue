	...
	<view class="page">
		...
		<view class="express">
			<view class="top ali-c">
				...
				<view class="text">
					<view class="text1">物流公司  :  {{wlInfo.post_name}}</view>
					<!-- <view class="text2">官方电话 {{wlInfo.exp_phone}}</view> -->
				</view>
			</view>
			<view class="right">
				<!-- {{wlInfo.post_name}} -->
				<view class="text">快递单号  :  {{wlInfo.post_no}}</view>
			</view>
		</view>
		...
		<view class="content bgf">
			<view>
				...
				<view class="flex list" :class="{one: index == 0 && wlInfo.delivery_status == 1}" v-for="(item, index) in wlInfo.list"
				 :key="index">
					<view class="time">
						<view class="day">{{item.time | dayfilter}}</view>
						<view>{{item.time | timefilter}}</view>
					</view>
					<view class="info flex1">
...
<script>
	export default {
		...
		filters:{
			dayfilter(time){
				let dayy = time.slice(0,11)
				return dayy
			},
			timefilter(time){
				let yy = time.slice(11)
				return yy
			}
		},
		onLoad(option) {
		...
		methods: {
			_getlogistics(){
				...
				this.$common.requests('/api/index/query_express',obj).then(res=>{
						let myinfo = JSON.parse(res.data[1])
					if(res.data[0].code == 1){
						let logisticsinfo = res.data[0].data
						if(logisticsinfo.state != 3){
							this.wlInfo.delivery_status = 2
						}else{
							this.wlInfo.delivery_status = 1
						}
						this.wlInfo.post_name = logisticsinfo.com
						this.wlInfo.post_no = logisticsinfo.nu
						this.wlInfo.addr = myinfo.address
						this.wlInfo.list = logisticsinfo.data
